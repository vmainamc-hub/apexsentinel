...
function sendWithTimeout<T = any>(send: (payload: any) => Promise<T>, payload: any, timeoutMs = REQUEST_TIMEOUT_MS): Promise<T> {
    return Promise.race([
        send(payload),
        new Promise<T>((_, reject) => window.setTimeout(() => reject(new Error('Deriv request timed out. Check the connection and try again.')), timeoutMs)),
    ]);
}
...
export default observer(function DTrader() {
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                ...
                const res = await sendWithTimeout(chart_api.api.send.bind(chart_api.api), payload);
    ...
    async function buy() {
        ...
        try {
            ...
            if (['DIGITOVER','DIGITUNDER','DIGITMATCH','DIGITDIFF','HIGHER','LOWER','TOUCH','NOTOUCH'].includes(type)) payload.barrier = barrier;
            const fresh = await sendWithTimeout(chart_api.api.send.bind(chart_api.api), payload);
            const p = fresh?.proposal;
            if (!p?.id) throw new Error(fresh?.error?.message || 'Could not get a live price for this contract right now.');
            const res = await sendWithTimeout(chart_api.api.send.bind(chart_api.api), { buy: p.id, price: Number(p.ask_price) });
    ...
    async function sell(contractId: number) {
        try {
            await sendWithTimeout(chart_api.api.send.bind(chart_api.api), { sell: contractId, price: 0 });
